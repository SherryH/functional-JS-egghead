...
const eitherIsNull = currentUser => {
  return currentUser == null ? Left(currentUser) : Right(currentUser);
};

const openSiteFn = currentUser => {
  return eitherIsNull(currentUser).fold(showLogin, () =>
...
// mocked functions
const defaultPrefs = x => `defaultPrefs(${x})`;
...
const getPrefsFn = user =>
  (user.premium ? Right(user) : Left("not premium")).fold(defaultPrefs, user =>
...
console.log(
  `getPrefsFn({premium: true, preferences: 'some_prefs'}) : `,
  getPrefsFn({ premium: true, preferences: "some_prefs" })
);
...
const streetNameFn = user =>
  eitherIsNull(user.address)
    .chain(addrs => eitherIsNull(addrs.street))
...
const concatUniqFn = (x, ys) =>
  eitherIsNull(ys.filter(y => y === x)[0]).fold(() => ys.concat(x), () => ys);
// const concatUniqFn = (x, ys) =>
...
const wrapExampleFn = example =>
  eitherIsNull(example.previewPath)
    .chain(path => tryCatch(() => fs.readFileSync(path)))
    .chain(c => tryCatch(() => c.toString()))
    .fold(() => example, preview => ({ ...example, preview }));
// const wrapExampleFn = example =>
...
// testing
console.log(
  `wrapExampleFn({previewPath: 'no_file.txt'}) : `,
  wrapExampleFn({ previewPath: "no_file.txt" })
);
console.log(
  `wrapExampleFn({previewPath: 'preview.txt'}) : `,
  wrapExampleFn({ previewPath: "preview.txt" })
);
...
const parseDbUrlFn = cfg =>
  tryCatch(() => JSON.parse(cfg))
    .chain(c => eitherIsNull(c.url))
    .fold(
      () => null,
      url => url.match(/postgresql:\/\/([^:]+):([^@]+)@([^:]+)/)
    );
